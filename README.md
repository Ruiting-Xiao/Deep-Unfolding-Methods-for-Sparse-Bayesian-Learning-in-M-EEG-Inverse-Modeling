...
本项目代码遵循"生成数据 → 预处理 → 训练模型 → 评估结果"的标准流程。


### 1. 前向模型生成（generate_model.py）

生成 EEG 源定位所需的四个核心文件：
- 训练集/验证集的引导场矩阵（Lead Field）与源位置


### 2. 数据集生成（generate_dataset.py）
...
**主要特性：** 支持GPU加速、批量存储、可配置参数、种子控制。


### 3. 数据预处理（process_normalize_data.py）

对 EEG 数据集进行归一化预处理，保证数值稳定性。

**输出文件：**

| 文件 | 说明 |
|------|------|
| `{prefix}_X_norm_*.npy` | 归一化源信号 |
| `{prefix}_Y_norm_*.npy` | 归一化传感器测量值 |
| `{prefix}_Lambda_norm_*.npy` | 归一化噪声协方差 |
| `global_G_norm_{prefix}.npy` | 归一化引导场矩阵 |



### 4. 模型训练

#### train_lsbl_em.py（EM模式）

基于 EM 更新的 L-SBL 神经网络训练。

- **更新规则**：两项加权组合，权重通过 Softplus 确保非负
- **损失函数**：MSE / MAE / 自适应损失
- **训练策略**：逐层训练 + 权重复制初始化 + 子批次处理

#### train_combined_model.py（组合模式）

组合 MacKay、Modified MacKay 与 EM 三种更新策略。

- **更新规则**：三项加权组合
- **权重约束**：非负且和为 1
- **三种更新规则**：MacKay、Modified MacKay、EM
...
| 脚本 | 说明 |
|------|------|
| `evaluation_train_data_EM.py` | 评估 EM 模型，计算 MSE/MAE/RMSE/RMAE/EMD 等指标 |
| `evaluation_train_data_Combined.py` | 评估组合模型，计算 8 项核心指标 |

### 6. 代码依赖关系

| 阶段 | 脚本 | 依赖 |
|------|------|------|
| 1 | `generate_model.py` | 无 |
| 2 | `generate_dataset.py` | 需要 `generate_model.py` 生成的引导场矩阵 |
| 3 | `process_normalize_data.py` | 需要 `generate_dataset.py` 生成的原始数据 |
| 4 | `train_lsbl_em.py` / `train_combined_model.py` | 需要 `process_normalize_data.py` 生成的归一化数据 |
| 5 | `evaluation_train_data_EM.py` / `evaluation_train_data_Combined.py` | 需要对应的训练脚本生成的模型检查点 |
---
...
## 四、 自适应反演网络（Adaptive InversionNet）
